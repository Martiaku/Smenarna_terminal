...
def transfer_money():
    while True:
        ...
        in_CZK = amount * BUY_RATES[currency_input]
        result = in_CZK / SELL_RATES[currency_output]
        ...
        print(
            f"\nPřevod: {amount} {NAME_CURRENCY[currency_input]} = {result:.2f} "
            f"{NAME_CURRENCY[currency_output]} při kurzu {SELL_RATES[currency_output]:.2f} Kč/"
            f"{NAME_CURRENCY[currency_output]}"
        )
        print(
            f"Použité kurzy: nákup {NAME_CURRENCY[currency_input]} = {BUY_RATES[currency_input]:.2f} Kč, "
            f"prodej {NAME_CURRENCY[currency_output]} = {SELL_RATES[currency_output]:.2f} Kč"
        )
        continue_prompt()
